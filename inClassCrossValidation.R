...
# and we use the mean() function to calculate the MSE of the 196 observations in the # validation set. Note that the -train selects only the observations that are not in,
# the training set.

attach(Auto)
mean((mpg-predict(lm.fit,Auto))[-train]^2)
# ^^ this calculates the mean squared error (look at formula on slides)
...
# normally choose lowest MSE but no major difference between quad and cubic 

# If we choose different training set instead, then we will obtain somewhat different errors, 
# on the validation set.
# testing with a new sample set 
set.seed(2)
train = sample(392,196)
lm.fit <- lm(mpg~horsepower, data = Auto, subset = train) 
mean((mpg-predict(lm.fit,Auto))[-train]^2)
...
# model that predict mpg using quadratic function of horsepower performs better

# should try at least 3 or 4 samples 
